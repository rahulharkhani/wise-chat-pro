...
class WiseChatChannelsTab extends WiseChatAbstractTab {
	...
	public function channelsCallback() {
		...
		foreach ($summary as $key => $channel) {
			...
			$backupURL = $url.'&wc_action=backupChannel&tab=channels&channel='.urlencode($channel->channel);
			$backupPrivateURL = $url.'&wc_action=backupPrivateChannel&tab=channels&channel='.urlencode($channel->channel);
			$html .= sprintf(
				'<tr id="%s" class="%s" style="display: none;">
					<td colspan="5">
