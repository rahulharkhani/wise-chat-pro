...
<div class="{{ messageClasses }}" data-id="{{ messageId }}" data-chat-user-id="{{ messageChatUserId }}">
	...
	<div class="wcActionWrapper">
		...
		<a href="#" class="wcAdminAction wcReplyTo wcInvisible" data-id="{{ messageId }}" title="{{ messageReplyToMessage }}"></a>
	</div>
	...
	<span class="wcMessageContent">
		...
		<span class="wcMessageContentInternal" {% if isTextColorSetForMessage %}style="color:{{ textColor }}"{% endif isTextColorSetForMessage %}>{{ messageContent }}</span>
	</span>
</div>